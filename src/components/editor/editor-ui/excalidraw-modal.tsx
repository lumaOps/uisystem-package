...
import { Dialog, DialogClose, DialogContent } from '@/components/dialog/Dialog';
  ...
  function ShowDiscardDialog(): JSX.Element {
    return (
      <Dialog open={discardModalOpen} onOpenChange={setDiscardModalOpen}>
        <DialogContent>{t('Are you sure you want to discard the changes?')}</DialogContent>
        <DialogClose asChild>
          <CustomButton
            onClick={() => {
        ...
        <DialogClose asChild>
          <CustomButton onClick={() => setDiscardModalOpen(false)}>{t('Cancel')}</CustomButton>
        </DialogClose>
      </Dialog>
    );
  }
          ...
          <div className="h-full w-full" ref={excaliDrawModelRef} tabIndex={-1}>
            <div className="h-full w-full">
              ...
              <div className="absolute bottom-5 right-1/2 z-10 flex translate-x-1/2 gap-2">
                ...
                <CustomButton onClick={save}>{t('Save')}</CustomButton>
              </div>
            </div>
          </div>
        </div>
